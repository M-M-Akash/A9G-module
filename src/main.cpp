 ...
 * 
 */
#define SERVER_IP   "akash-7rd6.localhost.run"
#define SERVER_PORT  80
#define REGISTERED_SECRET_KEY   "33172090-0b8d-4a59-95c1-d62da98d1349"
...
void gps_testTask(void *pData)
{
    ...
    while(1)
    {
        if(isGpsOn)
        {
            ...
            snprintf(requestPath,sizeof(buffer2),"/api/sync-device-data/?id=%s&rsk=%s&timestamp=%d&lat=%f&lon=%f&speed=%f&bearing=%.1f&altitude=%f&accuracy=%.1f&batt=%.1f",
                                                    buffer,REGISTERED_SECRET_KEY,time(NULL),latitude,longitude,nmeafloat_2_float(gpsInfo->vtg.speed_kph),gpsInfo->gga.altitude,0.0,percent*1.0);
            if(Http_Get(SERVER_IP,SERVER_PORT,requestPath,buffer,sizeof(buffer)) <0 )
...
double nmeafloat_2_float (struct minmea_float value) {
if (value.scale) {
return 1.0*value.value/value.scale;
} else {
return 0;
}
}
